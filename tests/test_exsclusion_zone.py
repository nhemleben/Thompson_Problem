...
from known_optimal import KNOWN_SOLUTIONS, cartesian_to_spherical
...
def test_n2_antipodal():
    ...
    #We run into coordinate chart issues, will need to verify
    #later with different coordinate chart and bounds
...
def known_configurations_test(candidate,n):
    _set_iv_dps(40)
    model = build_taylor_model(n)
...
def test_n4():
    n = 4
    candidate = [cartesian_to_spherical(p) for p in KNOWN_SOLUTIONS[n]]
    known_configurations_test(candidate,n)
